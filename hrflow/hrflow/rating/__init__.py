...
class Rating:
    ...
    def post(
        ...
        """
        This endpoint allows you to rate a Profile (resp. a Job) for Job (resp. a Profile)
        as a recruiter (resp. a candidate) with a score between 0 and 1.
        Visit : https://developers.hrflow.ai/reference for more information.

        The job_key and the job_reference cannot be null at the same time in the Request Parameters.
        The same for the profile_key and profile_reference.

        Args:
            score:                  <float>
                                    The score is an evaluation fit between 0 to 1 .
                                    If you're using stars in your system please use the following conversion:
                                    5 stars = 1.0 , 4 stars=0.8 , 3 stars=0.6, 2 stars=0.4, 1 star= 0.2 .
            role:                   <string>
                                    Role of the user rating the job role in {recruiter, candidate, employee, manager}.
            board_key:              <string>
            ...
            job_reference:          <string>
                                    The Job reference chosen by the customer or an external system.
                                    If you use the job_key you do not need to specify the job_reference and vice versa.
            profile_key:            <string>
                                    profile identifier (key)
            profile_reference:      <string>
                                    The Profile reference chosen by the customer or an external system.
